//! Headless direct-vs-index gate for software25d (mirrors software3d's).
//! Asserts the direct u32 scene path equals the index path widened through the
//! same palette, bit-for-bit, on a fixed E1M2 pose (no fuzz/bleed in view).
...
fn load() -> Option<(LevelData, PicData)> {
    ...
    level.load("E1M2", |n| pics.flat_num_for_name(n), &wad, None, None);
    Some((level, pics))
}

fn render_index_u32(w: usize, h: usize) -> Option<Vec<u32>> {
    let (mut level, mut pics) = load()?;
    let mut r = Software25D::new(FOV, w as f32, h as f32, false, false);
...
#[test]
fn index_path_renders_nonblank() {
    let Some(a) = render_index_u32(320, 200) else {
        return;
    };
...
fn direct_u32_equals_index_widened() {
    ...
    assert_eq!(
        mismatches, 0,
        "direct must equal index ({mismatches} differ)"
    );
}
