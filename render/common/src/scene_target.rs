...
use crate::{BufferSize, DrawBuffer, SceneTarget};
...
impl<T: PixelFmt> DrawBuffer for PixelTarget<'_, T> {
    ...
    fn resolve(&mut self, _pal_lit: &PalLit<T>, _use_palette: usize) {}
}

impl<T: PixelFmt> SceneTarget for PixelTarget<'_, T> {
    type Texel = T;

    #[inline(always)]
    fn texel(&self, lit: u16) -> T {
        // SAFETY: lit is 0..=255 (caller skips u16::MAX); block is [T; 256].
        unsafe { *self.block.get_unchecked(lit as usize) }
    }

    #[inline(always)]
    fn put(&mut self, pos: usize, texel: T) {
        unsafe {
            *self.surface.get_unchecked_mut(pos) = texel;
        }
    }

    #[inline(always)]
    fn scene_fuzz(&mut self, dst_pos: usize, src_pos: usize, _colourmap6: &[usize; 256]) {
        unsafe {
